...
public class MySnakeFrame extends JFrame {
	...
	public boolean comer() {
		if (this.manzana.getManzana().get(0).getX() == this.snake.getListaCuadrados().get(0).getX()
				&& this.manzana.getManzana().get(0).getY() == this.snake.getListaCuadrados().get(0).getY()) {
			return true;
		}
		return false;
	}
	...
	public void comprobarEstado(int iAlto, int iAncho) {
		if (jugando && !pausado) {
			if (snake.estaMuerta(iAlto, iAncho)) {
				...
				snake.crecer();
				manzana.getManzana().add(1, new Cuadrado(manzana.sacarX(), manzana.sacarY(), 20, 1));
				manzana.getManzana().remove(0);
