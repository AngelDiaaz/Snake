...
public class Serpiente extends Manzana{
    //Constantes. No hay
    ...
    /**
     * Cuando se salga la serpiente del tablero que aparezca por la otra parte
     * @param iAlto Alto del tablero
     * @param iAncho Ancho del tablero
     */
    private void seHaSalido(int iAlto, int iAncho) {
        //Hacemos las comprobaciones sobre la cabeza
        Cuadrado cabeza = listaCuadrados.get(0);

        if(cabeza.getX() < 0) {
        	cabeza.setPosX(iAncho - 6);
        } else if (cabeza.getX() > iAncho) {
        	cabeza.setPosX(0);
        } else if (cabeza.getY() < 0 ) {
        	cabeza.setPosY(iAlto - 6);
        } else if (cabeza.getY() > iAlto) {
        	cabeza.setPosY(0);
        }
    }
    ...
    //controlamos el cambio de dirección //Aqui se puede cambiar las teclas para los controles de la serpiente
